...
/** Forward gears only (excludes 'R') — used for random sequence generation. */
const FORWARD_GEARS = GEARS.slice(0, -1);

function GearShiftQTE({ isControlling, onSolve }) {
  const [sequence] = useState(() =>
    Array.from({ length: 5 }, () => FORWARD_GEARS[Math.floor(Math.random() * FORWARD_GEARS.length)])
  ); // forward gears only (1–5)
